...
def feedback_to_delta(feedback_type: str) -> tuple[float, float]:
    """
    Transformă feedback-ul utilizatorului în ajustări pentru target.

    colder  -> modelul a prezis prea cald, deci scădem predicția
    correct -> păstrăm predicția
    warmer  -> modelul a prezis prea rece, deci creștem predicția
    """
...
def load_feedback_data() -> pd.DataFrame:
    if not os.path.exists(FEEDBACK_FILE):
        raise FileNotFoundError(
            f"Nu există {FEEDBACK_FILE}. Salvează mai întâi feedback din aplicație."
        )

    df = pd.read_csv(FEEDBACK_FILE)

    if df.empty:
        raise ValueError("Fișierul de feedback există, dar este gol.")
...
def retrain_model():
    ...
    print(f"Feedback-uri găsite: {len(personalized_df)}")

    synthetic_df = generate_training_data(rows=3000)

    # Repetăm datele personale ca să aibă greutate mai mare în model.
    # Dacă ai puține feedback-uri, altfel ar fi ignorate de datele sintetice.
