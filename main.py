...
class SmartWeatherAssistant(ctk.CTk):
    def __init__(self):
        ...
        # Salvam ultima cautare pentru a putea trimite feedback.
        self.current_weather_data = None
        self.current_ml_prediction = None

        self.configure(fg_color="#0f172a")

        # Facem layout-ul principal responsive.
    ...
    def create_widgets(self):
        ...
        self.feedback_card.grid(row=2, column=0, columnspan=2, sticky="ew", pady=10)
        ...
        self.forecast_card.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=10)
        ...
        self.empty_forecast_label = ctk.CTkLabel(
            self.forecast_list_frame,
            text="Forecast-ul va aparea aici.",
            font=("Arial", 15),
            text_color="#94a3b8",
        )
        self.empty_forecast_label.grid(row=0, column=0, sticky="w", pady=8)
    ...
    def set_loading(self, is_loading: bool):
        if is_loading:
            self.search_button.configure(state="disabled", text="Se incarca...")
            self.status_label.configure(
    ...
    def display_forecast(self, forecast: list[dict]):
        # Stergem randurile vechi din forecast.
        ...
        for index, item in enumerate(forecast):
            ...
            desc_label = ctk.CTkLabel(
                ...
                text=(
                    f"{item['description']} | "
                    f"Vant: {item['wind_speed']} m/s | "
                    f"Umiditate: {item['humidity']}%"
                ),
                font=("Arial", 13),
                text_color="#94a3b8",
            )
    ...
    def submit_feedback(self, feedback_type: str):
        if self.current_weather_data is None or self.current_ml_prediction is None:
            self.status_label.configure(
                text="Cauta mai intai vremea pentru un oras.", text_color="#facc15"
            )
            return
    ...
    def show_error(self, message: str):
        self.weather_label.configure(text="Nu s-au putut incarca datele.")
        ...
        error_label = ctk.CTkLabel(
            self.forecast_list_frame,
            text="Nu exista forecast disponibil.",
            font=("Arial", 15),
            text_color="#94a3b8",
        )
        error_label.grid(row=0, column=0, sticky="w", pady=8)
